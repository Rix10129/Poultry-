    ...
    <div className="space-y-6">
      ...
      {returns.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="font-medium text-slate-600">No sale returns yet</p>
          <p className="text-sm text-slate-400 mt-1">Create a sale return when a customer returns goods</p>
          <Link href="/sales/returns/new" className="mt-4">
              ...
              {returns.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-700">
                    {r.customer?.name ?? <span className="text-slate-400 italic">Walk-in</span>}
                  </td>
