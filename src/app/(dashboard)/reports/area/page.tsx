...
export const metadata = { title: "Area-wise Sales Report" }
...
export default async function AreaReportPage({ searchParams }: Props) {
  ...
  // Fetch invoices with customer area in range
  ...
  for (const inv of invoices) {
    ...
    const area = inv.customer.area?.trim() || "No Area"
    ...
    if (existing) {
      existing.customerIds.add(inv.customer.id)
      ...
      areaMap.set(area, {
        area,
        customerIds: new Set([inv.customer.id]),
        invoiceCount: 1,
        totalSales: net,
        totalCollected: paid,
        outstanding: balance,
  ...
  const rows = Array.from(areaMap.values()).sort((a, b) => b.totalSales - a.totalSales)
  const totals = {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <ReportExportButton report="area" />
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Area-wise Report</h1>
          <p className="text-sm text-slate-500 mt-0.5">Sales and recovery grouped by customer area</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Areas",       value: rows.length.toString(),             color: "border-slate-200 bg-slate-50 text-slate-700" },
      ...
      {rows.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <p className="font-medium text-slate-600">No sales with customers in this period</p>
          <p className="text-sm text-slate-400 mt-1">
            Make sure customers have their Area field filled in
          </p>
        </div>
      ) : (
              ...
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.area} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right text-slate-500">{row.customerIds.size}</td>
      ...
      <p className="text-xs text-slate-400">
        Area is taken from the customer record. Customers without an area are grouped as "No Area". Cash sales without a customer are excluded.
      </p>
    </div>
  )
}
