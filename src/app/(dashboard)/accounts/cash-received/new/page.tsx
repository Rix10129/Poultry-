...
import { CashReceivedForm } from "@/components/accounts/cash-received-form"
...
// control accounts are excluded too — crediting them directly here would
...
export default async function NewCashReceivedPage() {
  ...
  const systemAccountIds = {
    ...
    CHEQUES_RECEIVABLE: accounts.find(a => a.code === SYSTEM_ACCOUNTS.CHEQUES_RECEIVABLE.code)?.id ?? "",
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">Cash Received</h1>
          <p className="text-sm text-slate-500">Record money coming in</p>
        </div>
      </div>

      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <CashReceivedForm customers={customers} otherAccounts={otherAccounts} systemAccountIds={systemAccountIds} />
      </div>
    </div>
  )
}
