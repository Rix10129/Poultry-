...
export async function GET(req: NextRequest) {
  ...
  await db.company.update({
    where: { id: company.id },
    data: { status: "ACTIVE", approvalToken: null },
  })

  const owner = company.users[0]
  if (owner) {
    await sendApprovalConfirmationEmail(owner.email, owner.name, company.name).catch(() => null)
  }

  return html(
    "✅ Approved",
    `<strong>${company.name}</strong> has been activated. The owner has been notified by email.`,
    "#16a34a"
  )
}
