...
export async function GET(req: NextRequest) {
  ...
  const products = await db.product.findMany({
    where: { companyId: authorization.actor.companyId, isActive: true, ...(categoryId ? { categoryId } : {}), ...(species ? { species: species as never } : {}) },
    include: { category: { select: { name: true } }, batches: { where: showZero ? {} : { quantity: { gt: 0 } }, orderBy: { expiryDate: "asc" } } }, orderBy: { name: "asc" },
  })
  const rows = products.flatMap((product: any) => product.batches.map((batch: any) => {
    const days = daysUntilExpiry(batch.expiryDate)
    return { product: product.name, category: product.category?.name ?? "—", species: product.species, batch: batch.batchNumber,
      expiry: batch.expiryDate.toISOString().slice(0, 10), available: batch.quantity,
